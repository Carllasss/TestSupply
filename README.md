...
- `docker-compose.yml` (в корне) — полный стек: Postgres, Redis, Qdrant,
  backend, frontend каталога, лендинг, nginx-реверс-прокси
  (`nginx/nginx.conf`) — это то, что рассчитано на реальный сервер.
- `catalog/docker-compose.yml` — тот же каталог отдельно, без лендинга и
  без reverse-proxy (публикует порты бэкенда/фронта напрямую) — удобно
  для разработки только каталога.
