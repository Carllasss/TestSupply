...
- Юнит-тесты — чистые функции без БД/Qdrant: нормализация имён/телефонов,
  дедупликация по домену/имени/телефону, ранжирование семантического
  поиска, поиск победителя в AI-рекомендации, защита от SSRF, URL-валидация
  схем в карточках, парсинг DuckDuckGo-ссылок.
- `tests/golden_set.json` + `tests/test_golden_set.py` — регрессионный
  набор запросов к поиску (например, «пицца моцарелла» не должна находить
  упаковку или соусы) против **живого** бэкенда — нужны реальные эмбеддинги
  и данные, моками это не проверить. Указывается через
  `GOLDEN_SET_BASE_URL` (по умолчанию `http://localhost:8080`); без
  доступного бэкенда эти тесты просто скипаются. Можно гонять и отдельно,
  с более читаемым выводом: `python3 tests/test_golden_set.py <base_url>`.
